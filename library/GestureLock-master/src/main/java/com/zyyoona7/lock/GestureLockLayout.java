...
public class GestureLockLayout extends RelativeLayout {
    ...
    private void handleResetMode() {
        if (mAnswerList.size() <= 0) {
            ...
            if (isAnswerRight) {
                ...
                toggleLockViewMatchedState(true);
                if (mOnLockResetListener != null) {
                    mOnLockResetListener.onSetPasswordFinished(true, mChooseList);
                }
            } else {
                //两次没密码不正确
                toggleLockViewMatchedState(false);
                if (mOnLockResetListener != null) {
                    mOnLockResetListener.onSetPasswordFinished(false, mChooseList);
                }
            }
        }
    }
    ...
    public void setAnswer(ArrayList<Integer> mList) {
        mAnswerList.clear();
        mAnswerList.addAll(mList);
    }

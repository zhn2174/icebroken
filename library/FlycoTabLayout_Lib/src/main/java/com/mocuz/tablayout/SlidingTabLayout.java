...
import com.mocuz.tablayout.utils.UnreadMsgUtils;
import com.mocuz.tablayout.widget.MsgView;
 ...
 * tl_indicator_color              设置下划线颜色
 * tl_indicator_height             设置下划线高度
 * tl_indicator_width              设置下划线固定宽度
 * tl_indicator_margin_left        设置下划线margin
 * tl_indicator_margin_top         设置下划线margin
 * tl_indicator_margin_right       设置下划线margin
 * tl_indicator_margin_bottom      设置下划线margin
 * tl_indicator_corner_radius      设置下划线圆角弧度
 * tl_indicator_gravity            设置下划线上方(TOP)还是下方(BOTTOM),只对常规下划线有用
 * tl_indicator_style              设置下划线为常规(NORMAL)或三角形(TRIANGLE)或背景色块(BLOCK)
 * <p>
 * tl_underline_color              设置底线颜色
 * tl_underline_height             设置底线高度
 * tl_underline_gravity            设置底线上方(TOP)还是下方(BOTTOM)
 * <p>
 * tl_divider_color                设置分隔线颜色
 * tl_divider_width                设置分隔线宽度
 * tl_divider_padding              设置分隔线的paddingTop和paddingBottom
 * <p>
 * tl_tab_padding                  设置tab的paddingLeft和paddingRight
 * tl_tab_space_equal              设置tab大小等分
 * tl_tab_width                    设置tab固定大小
 * <p>
 * tl_textsize                     设置字体大小
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        //对Canvas画出来的View进行适配
        mTextsize = AutoPX(mTextsize);
        mIndicatorMarginBottom = AutoPX(mIndicatorMarginBottom);
        mIndicatorMarginLeft = AutoPX2(mIndicatorMarginLeft);
        mIndicatorMarginRight = AutoPX2(mIndicatorMarginRight);
        mIndicatorHeight = AutoPX2(mIndicatorHeight);
        ta.recycle();
    }

    private float AutoPX(float px) {
        DisplayMetrics dm = getResources().getDisplayMetrics();
        return px * dm.heightPixels / 1920;
    }

    private float AutoPX2(float px) {
        DisplayMetrics dm = getResources().getDisplayMetrics();
        return px * dm.widthPixels / 1080;
    }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            addTab(i, null != pageTitle ? pageTitle.toString() : "", tabView);
        }
...
//                canvas.drawLine(paddingLeft + tab.getRight(), height/2, paddingLeft + tab.getRight(), height/2+1, mDividerPaint);
                canvas.drawCircle(paddingLeft + tab.getRight(), height / 2, 4.5f, mDividerPaint);
            }
        }
        if (mDividerWidth <= 3 && mDividerWidth > 0) {
            mDividerPaint.setStrokeWidth(mDividerWidth);
            mDividerPaint.setColor(mDividerColor);
            for (int i = 0; i < mTabCount - 1; i++) {
                View tab = mTabsContainer.getChildAt(i);
    ...
    public void setTabWidthNum(int num) {
        //设置view宽度平铺
        WindowManager wm = (WindowManager) getContext()
                .getSystemService(Context.WINDOW_SERVICE);
        int width = wm.getDefaultDisplay().getWidth();
        this.mTabWidth = width / num;
        updateTabStyles();
    }

    public void setTabWidthpx(int px) {
        //设置view宽度平铺
        this.mTabWidth = AutoPX(px);
        updateTabStyles();
    }
    ...
    public void setIndicatorWidthPX(int indicatorWidth) {
        this.mIndicatorWidth = AutoPX2(indicatorWidth);
        invalidate();
    }

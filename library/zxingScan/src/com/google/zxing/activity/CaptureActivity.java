		...
		public boolean handleMessage(Message msg) {
			switch (msg.what) {
				case PARSE_BARCODE_SUC: // 解析图片成功
					ToastUitl.showShort("解析成功，结果为：" + msg.obj);
					if (isfive) {
						new RxManager().post("FIVECODE", msg.obj.toString());
					} else {
						new RxManager().post("WEBURL",  msg.obj.toString());
					}
					finish();
					break;
				case PARSE_BARCODE_FAIL:// 解析图片失败
					ToastUitl.showShort("解析图片失败");
					break;
...
//	}
	...
	public void onCreate(Bundle icicle) {
		...
		isfive = getIntent().getBooleanExtra("isfive", false);
	...
	public void handleDecode(Result rawResult, Bundle bundle) {
		...
		if (isfive) {
			new RxManager().post("FIVECODE", rawResult.getText());
		} else {
			new RxManager().post("WEBURL", rawResult.getText());
		}
		finish();
	...
	private void displayFrameworkBugMessageAndExit() {
		...
		builder.setMessage("获取权限失败，请去设置中打开摄像头权限\n打开之后按两次返回键可回到该应用哦");

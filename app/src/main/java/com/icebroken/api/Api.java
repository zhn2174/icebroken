...
/*
   1. noCache 不使用缓存，全部走网络
    ...
    //构造方法私有
    private Api(int hostType,String type) {
        //开启Log
        ...
        retrofit = new Retrofit.Builder()
                .client(okHttpClient)
                .addConverterFactory(JsonConverterFactory.create(gson, true))
...
//            header = "Basic " + Base64.encodeToString("wallet:secret".getBytes(), Base64.NO_WRAP);
//        }
//        LogUtils.logd("Bearer==" + header);
            ...
            headerInterceptor = new Interceptor() {
                ...
                public Response intercept(Chain chain) throws IOException {
                    Request build = chain.request().newBuilder()
                            .addHeader("Content-Type", "application/json")
                            .addHeader("Accept", "application/json")
                            .addHeader("ACCESS_TOKEN",  AppApplication.token)
                            .build();
                    return chain.proceed(build);
                }
            };
        ...
        retrofit = new Retrofit.Builder()
                .client(okHttpClient)
                .addConverterFactory(JsonConverterFactory.create(gson, false))
                .addCallAdapterFactory(RxJavaCallAdapterFactory.create())
                .baseUrl(ApiConstants.getHost(hostType))
                .build();
        movieService = retrofit.create(ApiService.class);
...
//            RequestBody oidBody = request.body();
